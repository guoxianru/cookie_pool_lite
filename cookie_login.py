...
class CookieLogin:
    ...
    def get_login(self):
        ...
        if res_apiid.status_code == 400:
            app.logger.error("接码平台登陆失败：400")
            return 400
        if res_apiid.status_code == 504:
            app.logger.error("接码平台登陆失败：504")
            return 504
        try:
            if res_apiid.json()["ret"] == 1:
                ...
                app.logger.error("接码平台登陆失败：账号密码错误")
                return 1
            else:
                app.logger.error("接码平台登陆失败：%s" % res_apiid.json()["msg"])
                return 0
        except:
            app.logger.error("接码平台登陆失败：%s" % traceback.format_exc())
            return 0
    ...
    def get_money(self, dama_apiid):
        ...
        if res_money.status_code == 400:
            app.logger.error("接码平台余额失败：400")
            return 400
        if res_money.status_code == 504:
            app.logger.error("接码平台余额失败：504")
            return 504
        try:
            dama_status = res_money.text.split("|")[0]
            if dama_status == "1":
                dama_money = res_money.text.split("|")[1].strip()
                app.logger.info("接码平台余额成功：%s" % dama_money)
                return dama_money
            else:
                app.logger.error("接码平台余额失败：%s" % res_money.text.split("|")[-1])
                return 0
        except:
            app.logger.error("接码平台余额失败：%s" % traceback.format_exc())
            return 0
    ...
    def get_phone(self, dama_apiid):
        ...
        if res_phone.status_code == 400:
            app.logger.error("获取接码平台手机号失败：400")
            return 400
        if res_phone.status_code == 504:
            app.logger.error("获取接码平台手机号失败：504")
            return 504
        try:
            dama_status = res_phone.text.split("|")[0]
            if dama_status == "1":
                dama_phone = res_phone.text.split("|")[1].strip()
                app.logger.info("获取接码平台手机号成功：%s" % dama_phone)
                return dama_phone
            else:
                app.logger.error("获取接码平台手机号失败：%s" % res_phone.text.split("|")[-1])
                return 0
        except:
            app.logger.error("获取接码平台手机号失败：%s" % traceback.format_exc())
            return 0
    ...
    def get_code(self, dama_apiid, dama_phone):
        ...
        if res_code.status_code == 400:
            app.logger.error("接收接码平台验证码失败：400")
            return 400
        if res_code.status_code == 504:
            app.logger.error("接收接码平台验证码失败：504")
            return 504
        try:
            ...
            if str(dama_status) == str(dama_phone):
                ...
                app.logger.info("接收接码平台验证码成功：%s" % dama_code)
                return dama_code
            else:
                app.logger.error("接收接码平台验证码失败：%s" % res_code.text.split("|")[-1])
                return 0
        except:
            app.logger.error("接收接码平台验证码失败：%s" % traceback.format_exc())
            return 0
    ...
    def lahei_phone(self, dama_apiid, dama_phone):
        ...
        if res_lahei.status_code == 400:
            app.logger.error("拉黑接码平台手机号失败：400")
            return 400
        if res_lahei.status_code == 504:
            app.logger.error("拉黑接码平台手机号失败：504")
            return 504
        try:
            dama_status = res_lahei.text.split("|")[0]
            if dama_status == "1":
                app.logger.info("拉黑接码平台手机号成功")
            else:
                app.logger.error("拉黑接码平台手机号失败：%s" % res_lahei.text.split("|")[-1])
        except:
            app.logger.error("拉黑接码平台手机号失败：%s" % traceback.format_exc())
        ...
        while 1:
            try:
                ...
                try:
                    ...
                    app.logger.error("验证码发送失败，手机号异常")
                    ...
                    try:
                        ...
                        try:
                            ...
                            app.logger.info("获取cookie +1")
                            self.browser.close()
                            self.browser.quit()
                            return cookie
            except:
                app.logger.error("模拟登陆异常：%s" % traceback.format_exc())
                time.sleep(600)
                self.browser.delete_all_cookies()
                continue
